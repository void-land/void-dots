...
WINE_PREFIX="$HOME/Games/.umu-prefixes/umu-shared"
LOG_FILE="$GAME_DIR/steam-default.log"

# UMU Settings
GAMEID="umu-default"
PROTONPATH="$HOME/.local/share/Steam/compatibilitytools.d/GE-Proton11-1"
...
validate() {
	...
	if [ ! -d "$PROTONPATH" ]; then
		error "Proton not found at: $PROTONPATH"
		exit 1
	fi

	if command -v gamemoderun &>/dev/null; then
		info "GameMode: Available"
		PROTON_GAMEMODE=1
	else
...
setup_environment() {
	...
	# ProtonGe|CachyOs ^10 optimizations
	...
	# export MANGOHUD_CONFIG="full,vsync=1,fps_limit=120,fps_limit_method=early,font_size=16,font_scale=1.0"
